module mhd
    ...
    ! グリッド数
    integer, parameter :: XN = 480, YN = 600+7
    !integer, parameter :: XN = 160, YN = 200+7
    ! ファイル出力回数
    integer, parameter :: PN = 200
    ! 保存変数の個数
    integer, parameter :: VN = 9
    ! 計算領域のサイズ
    real(8), parameter :: XL = 16.0d0, YL = 20.0d0
    real(8), parameter :: TL = 100.0d0
    ! グリッドサイズ
    real(8), parameter :: dx = XL/XN, dy = YL/(YN-7)
    ! CFL数
    ...
    subroutine bc(q)
        implicit none
        real(8), intent(inout) :: q(XN, YN, VN)
        q(:, 1, 1) =  q(:, 6, 1)
        q(:, 1, 2) =  q(:, 6, 2)
        q(:, 1, 3) = -q(:, 6, 3)
        q(:, 1, 4) =  q(:, 6, 4)
    ...
    subroutine muscly(ql, qr, q, i, j)
        ...
        integer :: k
        do k = 1, VN
            ql(k) = q(i, j-1, k)+0.5d0*minmod(q(i, j, k)-q(i, j-1, k), q(i, j-1, k)-q(i, j-2, k))
            qr(k) = q(i, j, k)-0.5d0*minmod(q(i, j+1, k)-q(i, j, k), q(i, j, k)-q(i, j-1, k))
        end do
    ...
    subroutine mp5y(ql, qr, q, i, j)
        ...
        real(8) :: qminl, qmaxl, qminr, qmaxr, q5l, q5r
        do k = 1, VN
            dll = q(i, j-3, k)-2.0d0*q(i, j-2, k)+q(i, j-1, k)
            dl = q(i, j-2, k)-2.0d0*q(i, j-1, k)+q(i, j, k)
            d = q(i, j-1, k)-2.0d0*q(i, j, k)+q(i, j+1, k)
            dr = q(i, j, k)-2.0d0*q(i, j+1, k)+q(i, j+2, k)
            dmml = minmod(dll, dl)
            dmm = minmod(dl, d)
            dmmr = minmod(d, dr)
            qull = q(i, j-1, k)+2.0d0*(q(i, j-1, k)-q(i, j-2, k))
            qulr = q(i, j, k)+2.0d0*(q(i, j, k)-q(i, j+1, k))
            !qull = q(i, j-1, k)+4.0d0*(q(i, j-1, k)-q(i, j-2, k))
            !qulr = q(i, j, k)+4.0d0*(q(i, j, k)-q(i, j+1, k))
            qav = 0.5d0*(q(i, j-1, k)+q(i, j, k))
            qmd = qav-0.5d0*dmm
            qlcl = q(i, j-1, k)+0.5d0*(q(i, j-1, k)-q(i, j-2, k))+4.0d0/3.0d0*dmml
            qlcr = q(i, j, k)+0.5d0*(q(i, j, k)-q(i, j+1, k))+4.0d0/3.0d0*dmmr
            qminl = max(min(q(i, j-1, k), q(i, j, k), qmd), min(q(i, j-1, k), qull, qlcl))
            qmaxl = min(max(q(i, j-1, k), q(i, j, k), qmd), max(q(i, j-1, k), qull, qlcl))
            qminr = max(min(q(i, j, k), q(i, j-1, k), qmd), min(q(i, j, k), qulr, qlcr))
            qmaxr = min(max(q(i, j, k), q(i, j-1, k), qmd), max(q(i, j, k), qulr, qlcr))
            q5l = (2.0d0*q(i, j-3, k)-13.0d0*q(i, j-2, k)+47.0d0*q(i, j-1, k)+27.0d0*q(i, j, k)-3.0d0*q(i, j+1, k))/60.0d0
            q5r = (2.0d0*q(i, j+2, k)-13.0d0*q(i, j+1, k)+47.0d0*q(i, j, k)+27.0d0*q(i, j-1, k)-3.0d0*q(i, j-2, k))/60.0d0
            ql(k) = median(q5l, qminl, qmaxl)
            qr(k) = median(q5r, qminr, qmaxr)    
        end do
    end subroutine mp5y
    ...
    subroutine dqdt(dqdtij, q, ch)
        ...
        ! 保存則の計算
        !$omp parallel do private(i, ir, k)
        do j = 4, YN-3
            do i = 1, XN
                ir = mod(i, XN)+1
                do k = 1, VN
                    dqdtij(i, j, k) = -(fx(ir, j, k)-fx(i, j, k))/dx-(fy(i, j+1, k)-fy(i, j, k))/dy
                end do
            end do
        end do
    end subroutine dqdt
